"""Merge production IC A/B shard checkpoints."""
...
from run_production_ic_ab import (  # noqa: E402
    HOLDOUT_START,
    OUT,
    SELECTION_END,
    build_variants,
...
def main() -> None:
    ...
    for period, start, end in (
        ...
        rows = []
        ref_daily = filter_daily(daily_by[ref], start=start, end=end)
        ref_stats = summarize_variant(ref, ref_daily, period)
        for name in names:
            daily = filter_daily(daily_by[name], start=start, end=end)
            row = summarize_variant(name, daily, period)
            row["calmar_delta_vs_no_ic"] = round(row["calmar"] - ref_stats["calmar"], 4)
            row["cagr_delta_vs_no_ic"] = round(
                float(row["cagr_pct"]) - float(ref_stats["cagr_pct"]), 2
            )
            row["maxdd_delta_vs_no_ic"] = round(
                float(row["max_drawdown_pct"]) - float(ref_stats["max_drawdown_pct"]), 2
            )
            row["worst_delta_vs_no_ic"] = round(
                float(row["worst_day_pct"]) - float(ref_stats["worst_day_pct"]), 2
            )
            if name != "no_ic" and "ic_w50_d12" in daily_by:
                ctrl = summarize_variant(
                    "ic_w50_d12",
                    filter_daily(daily_by["ic_w50_d12"], start=start, end=end),
                    period,
                )
                row["calmar_delta_vs_ic50"] = round(row["calmar"] - ctrl["calmar"], 4)
                row["cagr_delta_vs_ic50"] = round(
                    float(row["cagr_pct"]) - float(ctrl["cagr_pct"]), 2
                )
            rows.append(row)
        rows.sort(key=lambda r: (-float(r["calmar"]), -float(r["cagr_pct"])))
        write_summary(OUT / f"summary_{period}.json", rows)

    print(f"Merged production IC A/B -> {OUT}", flush=True)
