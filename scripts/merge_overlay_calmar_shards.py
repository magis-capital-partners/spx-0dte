"""Merge overlay Calmar structure shard checkpoints."""
...
from overlay_calmar_variants import HOLDOUT_START, SELECTION_END  # noqa: E402
from run_overlay_calmar_structure import (  # noqa: E402
    OUT,
    load_variants,
    out_root,
    work_dir,
...
def main() -> None:
    ...
    if args.shards <= 1:
        ckpt = load_checkpoint(out_root(args.phase) / "checkpoint.json")
        if not ckpt or not ckpt.get("complete"):
            raise SystemExit("Phase incomplete (single-process checkpoint missing)")
    ...
    hold = build_summaries(
        variants, daily_by, trade_agg, period="holdout", start=HOLDOUT_START
    )
    write_summary(root / "summary_full.json", full)
    write_summary(root / "summary_selection.json", sel)
    write_summary(root / "summary_holdout.json", hold)
    print(f"Merged phase={args.phase} -> {root} ({len(names)} variants)", flush=True)
