"""Why-Not-Look-At suite — all waves, date-sharded, checkpointed.

Anti-overfit:
  - Rank / tune only on selection (<= SELECTION_END)
  - Sealed holdout (>= HOLDOUT_START) reported after ranking

Usage:
  python scripts/run_why_not_look_at_suite.py --shard 0 --shards 8 --resume
...
STRADDLE_CONTRACTS = 8  # small research notional vs ~31 vertical baseline
...
def simulate_short_straddle_day(
    ...
    *,
    ...
    multiplier: int = 100,
    contracts: int = STRADDLE_CONTRACTS,
) -> Tuple[float, int, int]:
    """Lightweight short ATM straddle day PnL. Returns (net_pnl, trades, stopped)."""
    by_ts = group_quotes(quotes)
    if not by_ts:
        return 0.0, 0, 0
    timestamps = sorted(by_ts)
    signals_by = {s.timestamp: s for s in signals}
    entry_ts = None
    ...
    if entry_ts is None:
        return 0.0, 0, 0
    signal = signals_by.get(entry_ts)
    if mode == "straddle_rich":
        if signal is None or signal.straddle_residual_z <= 1.0:
            return 0.0, 0, 0
    ...
    if entry_credit <= 0:
        return 0.0, 0, 0
    stop_level = 2.0 * entry_credit if mode == "straddle_stop2x" else None
    stopped = False
    exit_debit = None
    for ts in timestamps:
        if ts < entry_ts:
        ...
        snap = by_ts[ts]
        try:
            # mark both legs at ask to buy back
    ...
    per = entry_credit - exit_debit
    fees = 4 * contracts * fee_per_contract  # open+close both legs
    net = per * contracts * multiplier - fees
    # overlay mode: scale to ~10% of vertical book contribution (already small contracts)
    if mode == "straddle_overlay":
        net *= 0.5
        contracts = max(1, contracts // 2)
    return round(net, 2), 1, 1 if stopped else 0
...
def run_suite(
    *,
    ...
    variant_meta = [
        {"phase": p, "variant": n, "policy": pk, "fed": fm, "structure": sm}
        for p, n, _, pk, fm, sm in variants
    ]
    ...
    print(
        f"Why-Not-Look-At shard {shard}/{shards}: {len(variants)} variants × {shard_days} OOS days "
    ...
    for oos_i in range(start_oos_offset, oos_end):
        ...
        for phase, name, cfg, policy_key, fed_mode, structure_mode in variants:
            policy = make_policy(policy_key, trailing_stop=trailing_stop)
            day_cfg = day_config_overrides(cfg, policy, signal=first_signal, policy_key=policy_key)
            if policy_key == "skew_hard_side" and first_signal is not None:
                if first_signal.skew_z > 0:
                    day_cfg = replace(day_cfg, allowed_sides="bull_put")
                else:
                    day_cfg = replace(day_cfg, allowed_sides="bear_call")
            day_cfg, policy = apply_fed_day(day_cfg, policy, fed_mode=fed_mode, is_fomc=is_fomc)

            if structure_mode.startswith("straddle"):
                # Overlay: production vertical + small straddle
                if structure_mode == "straddle_overlay":
                    result = simulate_day(quotes, signals, config=day_cfg, policy=policy)
                    rows = trades_to_rows(result.trades)
                    update_trade_agg(trade_agg[name], rows)
                    strap_pnl, strap_trades, strap_stopped = simulate_short_straddle_day(
                        quotes, signals, mode="straddle_rich"
                    )
                    # scale overlay contribution
                    strap_pnl *= 0.25
                    net = round(result.net_pnl + strap_pnl, 2)
                    trades_n = len(result.trades) + strap_trades
                    stopped_n = sum(1 for t in result.trades if t.stopped) + strap_stopped
                    ...
                    net, trades_n, stopped_n = simulate_short_straddle_day(
                        quotes, signals, mode=structure_mode
                    )
                    trade_agg[name]["trades"] += trades_n
                    trade_agg[name]["stopped"] += stopped_n
                    trade_agg[name]["total_pnl"] += net
                    if net > 0:
                        trade_agg[name]["wins"] += trades_n
                    daily_by[name].append(
                        {
                            "date": test_date,
                            "eligible": True,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Why-Not-Look-At suite (checkpointed).")
