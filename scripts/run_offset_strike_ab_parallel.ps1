# Offset-strike vertical A/B — parallel shards then merge + summarize.
param(
  [int]$Shards = 8,
...
$SuiteDir = Join-Path $Root "data\offset_strike_ab"
$LogDir = Join-Path $SuiteDir "logs"
New-Item -ItemType Directory -Force -Path $LogDir | Out-Null
New-Item -ItemType Directory -Force -Path $SuiteDir | Out-Null

Write-Host "Starting $Shards offset-strike A/B shards..."
$jobs = @()
for ($s = 0; $s -lt $Shards; $s++) {
  $log = Join-Path $LogDir ("offset_ab_shard_{0}.log" -f $s)
  $errLog = Join-Path $LogDir ("offset_ab_shard_{0}.err" -f $s)
  $argList = @(
    (Join-Path $Root "scripts\run_offset_strike_ab.py"),
    "--shard", "$s",
    "--shards", "$Shards",
    "--resume",
...
Write-Host "Merging..."
& $Python (Join-Path $Root "scripts\merge_offset_strike_ab_shards.py") --shards "$Shards"
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

Write-Host "Summarizing..."
& $Python (Join-Path $Root "scripts\summarize_offset_strike_ab.py")
exit $LASTEXITCODE
