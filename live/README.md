...
| **CLEAR_FLATTEN_HALT** | Create `data/live/<date>/CLEAR_FLATTEN_HALT`, then restart the executor. One-shot: lifts a recovered `flatten`/`error_flatten`/`kill_switch`/`flatten_incomplete` entry halt so a flatten that was *not* a risk event (e.g. a watchdog KILL during a controlled restart, 2026-08-05) can resume. Refuses unless the recovered book is empty **and** IB reports no same-day SPXW lots; never lifts PnL/account/stale/stop-count halts. File is consumed on startup. |
