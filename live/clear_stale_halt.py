"""Operator one-shot clear for sticky stale-quote entry halts.

Create ``data/live/<date>/CLEAR_STALE_HALT`` then restart the executor.
Only clears ``stale_quotes`` halt reasons. Never clears flatten, daily-loss,
...
CLEARABLE_STALE_REASONS = frozenset({"stale_quotes"})
