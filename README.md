...
<!-- canonical-metrics:start -->
Eligible-calendar OOS backtest with realism package (2019-01-07 → 2026-08-05, 1522 eligible days, metrics basis `eligible_only`):
**19.3% CAGR**, Calmar **1.89**, max DD **10.2%**, worst day **-6.65%**, Sharpe 1.67, stop rate 20.6%.
<sub>Auto-generated by `scripts/reconcile_readme_metrics.py` from `data/dashboard_runs/p3_poststop_cooldown_120/summary.json` (exported 2026-08-05). Do not hand-edit.</sub>
<!-- canonical-metrics:end -->

(Historical note: ~19.9% / ~2.3 / ~8.6% before the realism package — entry-slip
P&L fix, 120s stop clock, $0.25 stop slip, $1.25 fees — and ~16.3% / ~1.46 /
~11.2% on the pre-2026-08-05 export before the settlement-P&L fixes.)
...
- **Session-now (cloud):** sanitized live status publishes to a **public gist** (not Pages). See [`live/README.md`](live/README.md).
- **Rebuild locally:**
